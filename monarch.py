...
def warMenu():
    ...
    if warEdit == 1:
        ...
        if war1 == True:
            ...
            maps = int(input('Would you like to see the current map? input 1 for yes, input 2 for no. '))
            if maps == 1:
                x = turtles()
                grid(x, 2, 100)
            ...
            whereTo = int(input('Would you like to move troops before attacking? Input 1 for yes, input 2 for no. '))
            if whereTo == 1:
                militaryMenus()
            elif whereTo ==2:
            ...
            if whereTo == 1:
                ...
                if inList == False:
                    ...
                    if freeTroops > 0:
                        ...
                        attackforce+=freeTroopsDraw
                    theNeighbors = province_neighbors(provinceChoice, provincesClaimed['player'], 'playername')
                    print('You can now send this attackforce to an enemy province. Here are the enemy provinces you can attack.')
                    enemyProvince = int(input('Which province would you like to select?'))
...
def province_neighbors (provinceSelect, provincesowned, who):
    ...
    if who == 'playername':
        ...
        filtered = list(filter(filterfunc_1, filtered))
        filtered = list(filter(filterfunc_2, filtered))
    elif who == 'enemy1':
        filtered = list(filter(filterfunc, neighbors))
        filtered = list(filter(filterfunc_1, filtered))
        filtered = list(filter(filterfunc_2, filtered))
